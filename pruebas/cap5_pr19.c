/*
    ...
    modificacion cuenta palabras

    modificacion: mostrar un mensaje de palabra encontrada o no encontrada
*/
#include <stdio.h>
#include <ctype.h>
#include <string.h>
    ...
    while(getword(word,MAXWORD)!=EOF)
        if(isalpha(word[0]))
            if((n=binsearch(word,keytab,NKEYS))>=0){
                keytab[n].count++;
				printf(">>>>Palabra %s ha sido encontrada.",word);
			}
			else
				printf(">>>>La palabra %s no ha sido encontrada.",word);
		else
			printf(">>>>La palabra %s no comienza por un carácter permitido.",word);
	for (n = 0; n < NKEYS; n++)
		if (keytab[n].count > 0)
			printf("%4d %s\n",
